    ...
    <table class="table" style = "margin-left:-50px">
        <tr>    
            ...
            <td>
              ...
              <a class="btn btn-dark" class="form-control" style="color: #FFFFFF; height: 35px;" href="{{route('trasparenciaOrdemCompraNovo', $unidade[0]->id)}}" > Novo <i class="fas fa-check"></i></a>    
            </td>
        </tr>   
    </table>
    </form> 
                        ...
                        <div id="div" class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="font-size: 12px;">
                            @foreach($processo_arq as $prc_arq) 
                                @if($prc_arq->processo_id == $prc->id)
                                    <a id="div" class="dropdown-item" href="{{asset('../public/storage/')}}/{{$prc_arq->file_path}}" target="_blank">{{ $prc_arq->title }}</a>
                                @endif
                            @endforeach
                        </div>	
                        </a>
